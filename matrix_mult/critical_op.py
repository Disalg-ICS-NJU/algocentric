...
class MultCost(CriticalOP):
    '''相乘操作类

        对于矩阵连乘算法, 计算两个矩阵相乘是其关键操作.
        该类用于提供判断元素相等接口并维护比较操作的次数.
        '''

    def __call__(self, elem_a: int, elem_b: int, elem_c: int) -> int:
        '''判断元素相等操作.

        Args:
            elem_a (int): 待乘元素elem_a
            elem_b (int): 待乘元素elem_b
